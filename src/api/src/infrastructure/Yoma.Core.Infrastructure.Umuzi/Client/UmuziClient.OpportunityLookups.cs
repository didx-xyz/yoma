...
namespace Yoma.Core.Infrastructure.Umuzi.Client
{
  public sealed partial class UmuziClient
  {
    ...
    private List<Domain.Lookups.Models.Country> GetCountries(IEnumerable<string>? source)
    {
      ...
      if (result.Count == 0)
        throw new InvalidOperationException("Umuzi opportunity must contain at least one supported country");
    ...
    {
      if (string.IsNullOrWhiteSpace(intervalName) && !count.HasValue)
      ...
      if (string.IsNullOrEmpty(intervalName))
        throw new InvalidOperationException("Umuzi commitment interval is required");

      var interval = _timeIntervalService.GetByNameOrNull(intervalName)
        ?? throw new InvalidOperationException($"Umuzi commitment interval '{intervalName}' is not supported");

      if (!count.HasValue || count < 1 || count > short.MaxValue)
        throw new InvalidOperationException($"Umuzi commitment count '{count}' must be between 1 and {short.MaxValue}");

      return (interval, (short)count.Value);
    }
    ...
    private static List<string>? GetKeywords(OpportunityResponse item)
    {
    ...
    private static IEnumerable<string?> GetKeywordCandidates(OpportunityResponse item)
    {
      // Preserve Umuzi's explicit search terms and raw category terminology. Mapped Yoma categories,
