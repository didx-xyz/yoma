namespace Yoma.Core.Domain.Entity
{
  ...
  public enum Setting
  {
    ...
    Admin_Notification_ReferralProgram_LimitReached, // optional

    // Referral notifications (referrer)
    User_Referrer_Notification_ReferralLink_Usage_Completed,
    User_Referrer_Notification_Referral_AccessChange,
    User_Referrer_Notification_ReferralLink_LimitReached, // optional
  ...
  /*
      migrationBuilder.InsertData(
        ...
        values: new object?[,]
        {
          ...
          { "86AF5931-A2D4-472B-8B7E-67D39FA38B15", "User", "Admin_Notification_ReferralProgram_LimitReached", "Referral program reached its limit", "Notify when a referral program hits its global completion limit (terminal state, optional)", "Notifications", "Referrals", 23, $"[\"{Constants.Role_Admin}\"]", "false", "Boolean", true, false },

          // Notifications: Referrals (Referrer)
          { "4A7D9D94-4B94-40C0-96EE-8B147B6FA1E7", "User", "User_Referrer_Notification_ReferralLink_Usage_Completed", "Referral completed", "Notify the referrer when someone they invited completes the program and they earn a reward", "Notifications", "Referrals", 24, $"[\"{Constants.Role_User}\"]", "true", "Boolean", true, true },
          ...
          { "FA3BE61A-8C3E-4762-9E94-BE9C11E82911", "User", "User_Referrer_Notification_ReferralLink_LimitReached", "Referral link limit reached", "Notify the referrer when their referral link hits its usage limit (terminal state, optional)", "Notifications", "Referrals", 26, $"[\"{Constants.Role_User}\"]", "false", "Boolean", true, false },
