  ...
  public class OrganizationStatusService : IOrganizationStatusService
  {
    ...
    public List<Models.Lookups.OrganizationStatus> List()
    {
      if (!_appSettings.CacheEnabledByCacheItemTypesAsEnum.HasFlag(Core.CacheItemType.Lookups))
        return [.. _organizationStatusRepository.Query().OrderBy(o => o.Name)];

      var result = _memoryCache.GetOrCreate(CacheHelper.GenerateKey<Models.Lookups.OrganizationStatus>(), entry =>
      {
