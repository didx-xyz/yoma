...
#pragma warning restore CA1862 // Use the 'StringComparison' method overloads to perform case-insensitive string comparisons
      ...
      {
        if (_logger.IsEnabled(LogLevel.Information)) _logger.LogInformation("Scheduling of tenant creation skipped: Already scheduled with status '{status}' for entity type '{entityType}' and entity id '{entityId}'", existingItem.Status, entityType, entityId);
        return;
      }
    ...
    public async Task UpdateScheduleCreation(SSITenantCreation item)
    {
      ...
      switch (item.Status)
      {
        ...
        case TenantCreationStatus.Error:
          ...
          if (_appSettings.SSIMaximumRetryAttempts == 0 || //no retries
            (_appSettings.SSIMaximumRetryAttempts > 0 && item.RetryCount > _appSettings.SSIMaximumRetryAttempts)) break;
