...
namespace Yoma.Core.Domain.Opportunity.Validators
{
  public class OpportunitySearchFilterValidator : PaginationFilterValidator<OpportunitySearchFilterAdmin>
  {
    ...
    public OpportunitySearchFilterValidator()
    {
      ...
      RuleFor(x => x.Countries).Must(x => x == null || !x.Any() || x.All(id => id != Guid.Empty)).WithMessage("{PropertyName} contains empty value(s).");
      ...
      RuleFor(x => x.CommitmentIntervalsParsed)
          .Must((x, commitmentIntervals) => commitmentIntervals == null || !commitmentIntervals.Any() || commitmentIntervals.All(item => item.Id != Guid.Empty && item.Count >= 1))
