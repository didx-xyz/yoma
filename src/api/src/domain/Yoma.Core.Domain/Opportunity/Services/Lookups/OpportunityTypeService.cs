...
namespace Yoma.Core.Domain.Opportunity.Services.Lookups
{
  public class OpportunityTypeService : IOpportunityTypeService
  {
    ...
    {
      var result = GetByIdOrNull(id) ?? throw new ArgumentException($"{nameof(OpportunityType)} for '{id}' does not exists", nameof(id));
      return result;
    }
    ...
    public List<OpportunityType> List()
    {
      if (!_appSettings.CacheEnabledByReferenceDataTypes.HasFlag(Core.ReferenceDataType.Lookups))
        return _opportunityTypeRepository.Query().ToList();
