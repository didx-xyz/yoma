...
export async function getServerSideProps(context: GetServerSidePropsContext) {
  ...
  const { categoryId, storeId } = context.params as IParams;
  //const { query, page } = context.query;

  // 👇 prefetch queries on server
  await queryClient.prefetchQuery({
    queryKey: [`StoreCategoryItems_${categoryId}_${storeId}`],
    queryFn: () =>
      searchStores(
        {
...
const MarketplaceStoreItemCategories: NextPageWithLayout<{
  ...
  //page?: string;
  error: string;
}> = ({ /*query, page, */ categoryId, storeId, error }) => {
              ...
              <div className="grid grid-cols-1 gap-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {data.map((item, index) => (
                  <Link
                    ...
                    href={`/marketplace/${categoryId}/${storeId}/${item.id}`}
                  >
