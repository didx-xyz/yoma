...
import type { ProgramLinkReferrer } from "~/api/models/referrals";
import { getOrCreateReferralProgramReferrerLink } from "~/api/services/referrals";

interface ProgramReferrerLinkDetailsProps {
  programId: string;
  programName: string;
  link: ProgramLinkReferrer;
  showQRCode?: boolean;
...
export const ProgramReferrerLinkDetails: React.FC<
  ProgramReferrerLinkDetailsProps
> = ({
  programId,
  programName,
  link,
  ...
  const [linkWithQR, setLinkWithQR] = useState<ProgramLinkReferrer | null>(
    link.qrCodeBase64 ? link : null,
  );
  const [loadingQR, setLoadingQR] = useState(false);

  useEffect(() => {
    setShowQRCode(!!link.qrCodeBase64);
    setLinkWithQR(link.qrCodeBase64 ? link : null);
  }, [link]);
  ...
  const handleToggleQR = useCallback(async () => {
    ...
    if (!linkWithQR?.qrCodeBase64) {
      setLoadingQR(true);
      try {
        const fullLink = await getOrCreateReferralProgramReferrerLink(
          programId,
          true,
        );
  ...
  }, [linkWithQR, programId, showQRCode]);
          ...
          <div className="min-w-0">
            <div className="mb-2">
              ...
              <p className="text-base-content/60 mt-0.5 text-sm">
                Copy and share this program referrer link
              </p>
            </div>
