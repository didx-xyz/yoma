...
| ~~Provider framing of the hosted journey is unconfirmed~~ | ~~Medium~~ | **Resolved 2026-09-11**: IXO confirmed embedding is supported, the Test `/pay` response sets neither `X-Frame-Options` nor a framing CSP, and the page was seen rendering inside the iframe on Dev. The rest of the embedded journey past sign-in is still unproven — any downstream step that refuses framing goes to IXO as a specific failure |
