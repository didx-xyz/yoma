...
- ⚠️ **A `null` pool means the cap is NOT ENFORCED — unlimited — never "nothing can be awarded."** Every allocation path applies a level's cap only when that level has a pool: `ProcessRewardAllocation` (`OpportunityService.cs:2172-2194`) skips a level whose `rewardPool` is null, the referral path guards on `treasury.ZltoRewardPoolCurrentFinancialYear.HasValue` / `program.ZltoRewardPool.HasValue` (`LinkUsageService.cs:703-717`), and `TreasuryService.ZltoRewardAwarded` never checks a pool before accruing. With no pool at any level the configured rewards are paid in full, forever. Copy that reads an empty pool as a block is the exact inverse of what happens — see Decisions, 2026-08-07.
...
- [x] **T4** — Referral Rewards alignment (YOM-1073) + `?tab=referrals`: shared detail block + compact row, soft Treasury-capacity notice, `AdminProgramInfo`'s private reward formatters retired, Treasury **ZLTO reward** figures (never USD payout) *(dev complete)*
...
- [x] **Fix the merge artifact in `OpportunityCompletionRead.tsx`** — done in `0b3cb102`
- [x] **T4 corrective (pool-unset copy)** — "Not set — no ZLTO can be awarded" was the **inverse** of the server behaviour, on both the Treasury and the organisation stat. Now "…are not capped by the Treasury" / "…this organisation's awards are not capped". `OpportunityRewardContext`'s "No cap — unlimited" was already right
...
- **2026-08-07: an empty pool means UNLIMITED, and the UI said the opposite.** Found while verifying T4's ground truth, and material enough to correct outside T4's stated scope. Every allocation path applies a level's cap only when that level has a pool — `ProcessRewardAllocation` returns the reward untouched when `rewardPool` is null (`OpportunityService.cs:2172-2194`), the referral path guards on `treasury.ZltoRewardPoolCurrentFinancialYear.HasValue` and `program.ZltoRewardPool.HasValue` (`LinkUsageService.cs:703-717`), and `TreasuryService.ZltoRewardAwarded` accrues without ever consulting a pool. So a Treasury with no ZLTO pool does not stop rewards; it stops *capping* them. `TreasuryOverview` and `OrganizationRewardStats` both said "Not set — no ZLTO can be awarded", which would have an admin leave a pool blank believing it safe. Both notes replaced. `OpportunityRewardContext` already read "No cap — unlimited" and was correct. **Carry this forward into T5 and T6: never write copy that treats a null pool as a block.**
- **2026-08-07: T4 ships as a detail + row pair plus a soft capacity notice, with no new API contract.** `POST /referral/program/search/admin` returns the full `Program` (Admin role), so the row needed no new endpoint — unlike T3, there is no API gap here. Referral rewards are capped **Treasury (current FY) → Program (lifetime)**, referee funded before ambassador, partial payouts allowed; `lib/referral/rewardCapacity.ts` mirrors `CalculateProgramRewardEstimate` exactly so the derived figure can never disagree with the `zltoRewardEstimate` the API already returns. Because the server has **no** rule tying a program pool to Treasury capacity, the notice is advisory in all three homes — no field error, no disabled submit. On the edit wizard it drops the *programme* cap whenever the typed pool differs from the saved one, because a typed pool has no server-derived balance and the UI does not compute balances; understating the cap is safe, inventing a balance is not.
- **2026-08-07: `TreasuryZltoRewardStats` extracted from `TreasuryOverview`.** The referral detail block shows the Treasury's four ZLTO figures beside the programme's own, and duplicating them was how T0's whole "one place per figure" rule would have been lost. Overview now renders the extraction; the labels and tooltips are unchanged apart from the pool-unset note above.
- **2026-08-07: `Program.referrerBalance` added to the TS model.** The API has returned `ReferrerBalance` (limit − total) all along; `AdminProgramInfo` was subtracting it client-side. Same rule as the reward balances — server-derived, display-only. The create-form's default `Program` object needed the field too, which is how `tsc` found the one other construction site.
